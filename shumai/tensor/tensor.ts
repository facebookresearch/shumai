...
import { creationTracker, cyrb53, Float16Array, is_tidy_run } from '../util'
...
export class Tensor {
  ...
  private track_tidy_ptr(ptr?: number) {
    if (!ptr) ptr = this.ptr
    if (!creationTracker.has(ptr)) creationTracker.set(ptr, this)
  }
  ...
  constructor(obj) {
    if (obj.constructor === Tensor) {
      ...
      this.grad = obj.grad
      this.op = obj.op
      if (is_tidy_run) this.track_tidy_ptr()
      return
    }
    if (obj.hasOwnProperty('_ptr')) {
      this._injest_ptr(obj._ptr)
      if (is_tidy_run) this.track_tidy_ptr(obj._ptr)
      this.deps = obj._deps
      return
    }

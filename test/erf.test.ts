...
describe('erf', () => {
  it('basic', () => {
    ...
    expectArraysClose(
      ...
      [-0.2763264, 0.2763264, 0.5204999, 0.7111556, -0.4283924]
    )
  })
  ...
  it('blowup', () => {
    ...
    const r = sm.erf(t)
    expectArraysClose(<Float32Array>r.valueOf(), [-0.9522852, -0.999593, -0.9999883, -1])
  })

  it('scalar', () => {
    const t = sm.scalar(1)
    const r = sm.erf(t)
    expect(isClose(<number>r.valueOf(), 0.8427008)).toBe(true)
  })

  it('2D Tensor', () => {
    const t = sm.tensor(new Float32Array([0.2, 0.3, 0.4, 0.5])).reshape([2, 2])
    const r = sm.erf(t)
    expectArraysClose(<Float32Array>r.valueOf(), [0.2227026, 0.32862678, 0.42839235, 0.5204999])
  })

  it('propagates NaNs', () => {
    const a = sm.tensor(new Float32Array([0.5, NaN, 0]))
    const res = sm.erf(a)
    expectArraysClose(<Float32Array>res.valueOf(), [0.5204999, NaN, 0.0])
  })
